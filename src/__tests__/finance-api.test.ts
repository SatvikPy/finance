...
async function createUser(params: { username: string; role: "VIEWER" | "ANALYST" | "ADMIN"; password: string }) {
  const passwordHash = await bcrypt.hash(params.password, 10);
  return prisma.user.create({
    data: { username: params.username, passwordHash, role: params.role as any, status: "ACTIVE" },
  });
}
...
describe("Finance backend RBAC + dashboard analytics", () => {
  ...
  test("viewer cannot create transactions but can read dashboard summary", async () => {
    ...
    const loginRes = await request(app).post("/auth/login").send({ username: "viewer1", password });
  ...
  test("analyst can list transactions and sees filtered counts", async () => {
    ...
    const loginRes = await request(app).post("/auth/login").send({ username: "analyst1", password });
    expect(loginRes.status).toBe(200);
    const token = loginRes.body.token as string;
